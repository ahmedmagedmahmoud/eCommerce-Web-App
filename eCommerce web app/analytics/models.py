...
# To force sessions to be only one for a user :

# def post_save_session_receiver(sender, instance, created, *args, **kwargs):
#     if created:
#         qs = UserSession.objects.filter(user=instance.user, ended=False, active=False).exclude(id=instance.id)
#         for i in qs:
#             i.end_session() # end every session except the current logged in one
#         if not instance.active and not instance.ended: if user is not active and the session is not ended , then end it.
#             instance.end_session()
#
# post_save.connect(post_save_session_receiver, sender=UserSession)

# To end sessions of in active users :
...
#             for i in qs:
#                 i.end_session()  # end all the sessions of that in active not created user
#
#
...
def user_logged_in_receiver(sender, instance, request, *args, **kwargs):
    ...
    # we want to store the session key for the user to create a user session and store it in the data base
